...
{
    public class UserVerifyCurrentEmailCodeExampleFilter : IOperationFilter
    {
        public void Apply(OpenApiOperation operation, OperationFilterContext context)
        {
            ...
            if (controllerName != "User" || actionName != "VerifyEmailChangeCurrent")
            {
                return;
            }
            ...
            if (operation.Responses.ContainsKey("401"))
            {
                ...
                if (content != null)
                {
                    ...
                    {
                        Summary = "Mã xác thực đã hết hạn",
                        Value = new OpenApiString(
                        """
                        {
                    ...
                    {
                        Summary = "Mã xác thực không hợp lệ",
                        Value = new OpenApiString(
                        """
                        {
                          "message": "Xác thực thất bại",
                    ...
                    {
                        Summary = "Token không hợp lệ / Thiếu token",
                        Value = new OpenApiString(
                        """
                        {
                          "message": "Xác thực thất bại",
